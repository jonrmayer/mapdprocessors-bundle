...
public class GetMapDProcessorTest {
	...
	public void testService() throws InitializationException, IOException {
		final TestRunner runner = TestRunners.newTestRunner(GetMapDProcessor.class);
//		OutputStream testin = testavro();
		...
		runner.setProperty(GetMapDProcessor.MAPD_SERVICE, "service");
		
		runner.setProperty(GetMapDProcessor.SQL_STATEMENT, "select * from mapd_countries");
...
//		runner.enqueue(bytes);
		...
		runner.run();
		List<MockFlowFile> results = runner.getFlowFilesForRelationship(GetMapDProcessor.REL_SUCCESS);
